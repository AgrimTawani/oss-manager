# Documentation

Project docs for OSS Contribution Manager.

| Doc | Description |
|-----|-------------|
| [System architecture](architecture.md) | Components, data model, flows, and deployment topology |
| [Setup guide](setup.md) | Neon, GitHub OAuth, local dev, Vercel deploy, and GitHub Actions |
| [Environment variables](../.env.example) | `.env` template in the repo root |

## Quick links

- [Repository](https://github.com/AgrimTawani/oss-manager)
- [Production app](https://oss-manager-black.vercel.app)
- [Vercel project settings](https://vercel.com/agrimtawanis-projects/oss-manager/settings/environment-variables)
- [GitHub Actions secrets](https://github.com/AgrimTawani/oss-manager/settings/secrets/actions)
